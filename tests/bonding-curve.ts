...
import { createMint, createSyncNativeInstruction, getAssociatedTokenAddress, getAssociatedTokenAddressSync, getMint, getOrCreateAssociatedTokenAccount, mintTo, NATIVE_MINT, TOKEN_2022_PROGRAM_ID } from "@solana/spl-token"
...
import { getAuthAddress, getOrcleAccountAddress, getPoolAddress, getPoolLpMintAddress, getPoolVaultAddress } from "./utils";
...
describe("bonding_curve", () => {
  ...
  // const PROGRAM_ID = new anchor.web3.PublicKey("48nFfNHx1nYQ3xoGEwhQJPQvFV3cd4qcftUyR6dVR6Qi");
  const transactions = [];
  ...
  it("initialize proxy", async () => {
    ...
    //  In reality , you have to replace user to operating address which is generated automatically
    ...
    // submit transaction
    const txSignature = await sendAndConfirmTransaction(connection, transaction, [user]);
